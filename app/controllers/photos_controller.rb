class PhotosController < ApplicationController
  def index
    ...
    @photos = @q.result(:distinct => true).includes(:post).page(params[:page]).per(10)
    @location_hash = Gmaps4rails.build_markers(@photos.where.not(:location_latitude => nil)) do |photo, marker|
      ...
      marker.infowindow "<h5><a href='/photos/#{photo.id}'>#{photo.post_id}</a></h5><small>#{photo.location_formatted_address}</small>"
    end
  ...
  def create_row_from_post
    ...
    if @photo.valid?
      ...
      redirect_to("/posts/#{@photo.post_id}", notice: "Photo created successfully.")
    else
      render("photo_templates/new_form_with_errors.html.erb")
    end
  end
  ...
  def destroy_row_from_post
    @photo = Photo.find(params.fetch("id_to_remove"))

    @photo.destroy

    redirect_to("/posts/#{@photo.post_id}", notice: "Photo deleted successfully.")
  end
